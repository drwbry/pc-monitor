...
**Date:** 2026-05-26
**Status:** Implemented — v1.0.8 shipped 2026-05-27
...
- ~~No tray mode in v1~~ **Tray mode implemented in v1.0.7.** Minimize or close sends the window to the system tray. Right-click tray icon: Open / Exit. Double-click to restore.
...
- **Target framework:** `net8.0-windows10.0.19041.0` with `<UseWPF>true</UseWPF>` and `<UseWindowsForms>true</UseWindowsForms>` (WinForms used only for `NotifyIcon` tray support). Nullable enabled. WinForms implicit usings explicitly removed in the App csproj to prevent `System.Drawing`/`System.Windows.Media` namespace collisions.
- **CommunityToolkit.Mvvm 8.3.2** for view models/commands. Keep MVVM thin and boring; do not introduce a larger UI framework.
- **LibreHardwareMonitorLib 0.9.6** (MIT) for CPU package temp, per-core temps, throttle-related sensors when available. **Note:** On i9-14900HX (and likely other 14th gen Intel with Secure Boot), LHM's ring0 driver is blocked and all temperature values are null. Fallback: `Thermal Zone Information` Windows performance counter — requires elevation but no kernel driver.
- **`System.Diagnostics.PerformanceCounter`** package plus **WMI / CIM** for CPU%, RAM, disk, process listings, and event log queries. Process RAM uses `Win32_PerfFormattedData_PerfProc_Process.WorkingSetPrivate` (matching Task Manager's Memory column), cached every 5 seconds.
- **`System.Management` 10.0.2** — bumped from 9.0.0 to satisfy LHM 0.9.6's transitive dependency.
- **xUnit + FluentAssertions** for `Core` unit tests. 63 tests as of v1.0.8.
- **AssemblyName:** `PcMonitor` (not `PcMonitor.App`) so the published binary is `PcMonitor.exe`.
- **Custom icon:** dark rounded rect + green EKG pulse line. Multi-resolution ICO generated by `app/install/generate-icon.ps1`. Set via `<ApplicationIcon>icon.ico</ApplicationIcon>` in App csproj.
- **Version:** set in `app/Directory.Build.props` as `<Version>`. Displayed in title bar and window header. Current: 1.0.8.
...
- The app manifest remains `asInvoker` (no UAC prompt). Temperature sensors require elevation; the recommended install path is `install.ps1 -RegisterStartupTask`, which registers a Task Scheduler entry with `RunLevel Highest` that auto-launches the app elevated at login without a UAC prompt. Without elevation the app runs fine but shows `Pkg Temp --`.
...
- C: free space
- Top processes table (top 5 by **RAM**; one row each: name, CPU %, RAM). Sorted by RAM descending so persistent high-memory processes (VmmemWSL, chrome, Memory Compression) are always visible. Refreshes at 1 Hz. Processes are **grouped by name** — all `chrome.exe` instances appear as one row with summed CPU% and RAM. PID 0 (Idle) and PID 4 (System) are excluded (WMI returns junk WorkingSetPrivate for them).
...
- **Installer:** `app/install/install.ps1 -Publish [-RegisterStartupTask]` — publishes PcMonitor.exe, copies it to `%LocalAppData%\PcMonitor\`, copies companion scripts to `Documents\SysLogs\scripts\`, creates a Start Menu shortcut, and optionally registers an elevated login task. `app/install/publish.ps1` handles the `dotnet publish` step. No MSI, no admin for the installer itself (task registration requires an elevated shell once).
...
- **Tray mode implemented.** Minimize/close sends to system tray. `NotifyIcon` via `UseWindowsForms`. Open/Exit context menu. Double-click to restore.
- **No notifications / toasts / sounds.** Pull-based only.
